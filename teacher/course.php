<?php
	...
	$nquery=mysqli_query($conn,"SELECT course_id,course.course_thname,course.course_engname,course.course_thcode,course.course_engcode,course.course_year_mco2,univercity.univercity_thname,faculty.faculty_name,department.department_name,course.course_status 
    FROM course 
        ...
        <h3>ตารางแสดงข้อมูลหลักสูตร</h3>
            ...
            <table>
              ...
              <?php $i=0; while($row=mysqli_fetch_array($nquery)){ $i=$i+1 ?>
                <tr>
                  ...
                  <?php
                    if ($row["course_status"]==1) {
                      ...
                      <input class="form-check-input" type="checkbox" id="icon'.$row["course_id"].'" checked>
                    </div>';
                    }
                    else if ($row["course_status"]==0) {
                      echo '<div class="form-check form-switch">
                      <input class="form-check-input" type="checkbox" id="icon'.$row["course_id"].'">
                    </div>';
                    }
                ?>
                ...
                <script>
                    $(function() {
                      $('#icon<?php echo $row['course_id']; ?>').change(function() {
                        //alert($(this).prop('checked'));
                        var ch_val = $(this).prop('checked');
                        var rel = <?php echo $row['course_id']; ?>;
                        //alert(ch_val);
                        ...
                        $.ajax({
                            url: '../teacher/status/statuscourse.php',
                            type: 'POST',
...
$(document).ready(function(){  
    ...
    $(document).on('click', '.edit_data', function(){  
         ...
         {  
              $.ajax({  
                   url:"../BasicData/course/editlec.php",  
                   method:"POST",  
                   data:{employee_id:employee_id},  
                   success:function(data){  
    ...
    $(document).on('click', '.edit_status', function(){  
         ...
         {  
              $.ajax({  
                   url:"../BasicData/course/statuslec.php",  
                   method:"POST",  
                   data:{employee_id:employee_id},  
                   success:function(data){  

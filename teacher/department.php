<?php
	...
	$nquery=mysqli_query($conn,"SELECT department_id,univercity.univercity_thname,faculty.faculty_name,department.department_name,department.department_status
  FROM department
        ...
        <h3>ตารางแสดงข้อมูลภาควิชา</h3>
            ...
            <table>
              ...
              <?php $i=0; while($row=mysqli_fetch_array($nquery)){ $i=$i+1 ?>
                <tr>
                  ...
                  <?php
                    if ($row["department_status"]==1) {
                      ...
                      <input class="form-check-input" type="checkbox" id="icon'.$row["department_id"].'" checked>
                    </div>';
                    }
                    else if ($row["department_status"]==0) {
                      echo '<div class="form-check form-switch">
                      <input class="form-check-input" type="checkbox" id="icon'.$row["department_id"].'">
                    </div>';
                    }
                ?>
                ...
                <script>
                    $(function() {
                      $('#icon<?php echo $row['department_id']; ?>').change(function() {
                        //alert($(this).prop('checked'));
                        var ch_val = $(this).prop('checked');
                        var rel = <?php echo $row['department_id']; ?>;
                        //alert(ch_val);
            ...
            <td>
                  <button type="button" name="view" value="view" data-bs-target="#staticBackdrop" id="<?php echo $row["department_id"]; ?>" class="btn btn-info btn-xs view_data"><i class='far fa-eye'></i></button>
            </td>  
                </tr>
                <?php } ?>
              </tbody>

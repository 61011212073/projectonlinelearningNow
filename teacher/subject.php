    ...
    <section>
      <div class="container-fluid">
        <h3>ตารางแสดงข้อมูลรายวิชา</h3>
                ...
                <script>
                    $(function() {
                      $('#icon<?php echo $row['subject_id']; ?>').change(function() {
                        ...
                        $.ajax({
                            url: '../teacher/status/statussubject.php',
                            type: 'POST',
            ...
            <td>
                  <button type="button" name="view" value="view" data-bs-target="#staticBackdrop" id="<?php echo $row["subject_id"]; ?>" class="btn btn-info btn-xs view_data"><i class='far fa-eye'></i></button>
            </td>  
                </tr>
                <?php } ?>
              </tbody>
...
$(document).ready(function(){  
    ...
    $(document).on('click', '.edit_data', function(){  
         ...
         {  
              $.ajax({  
                   url:"../BasicData/subject/editlec.php",  
                   method:"POST",  
                   data:{employee_id:employee_id},  
                   success:function(data){  
    ...
    $(document).on('click', '.edit_status', function(){  
         ...
         {  
              $.ajax({  
                   url:"../BasicData/subject/statuslec.php",  
                   method:"POST",  
                   data:{employee_id:employee_id},  
                   success:function(data){  
...
<div id="dataModal1" class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">  
      <div class="modal-dialog">  
           <div class="modal-content">  
                <div class="modal-header">  
                     <!-- <button type="button" class="close" data-dismiss="modal">&times;</button>   -->
                     <h4 class="modal-title"  id="staticBackdropLabel">แก้ไขข้อมูลรายวิชา</h4>  
                     <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>  
